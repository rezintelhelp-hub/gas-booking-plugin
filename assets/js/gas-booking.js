...
jQuery(document).ready(function($) {
    ...
    function formatPrice(amount, currencyCode) {
        ...
        currencyCode = gasBooking.currency || currencyCode;
        var symbols = {
    ...
    function renderRoomDetails(room, images, amenities, checkin, checkout, guests, occSettings) {
        var currency = room.currency || gasBooking.currency || 'EUR';
    ...
    function calculatePrice(unitId, checkin, checkout, guests, adults, children) {
        ...
        $.ajax({
            ...
            success: function(response) {
                var currency = $roomWidget.data('currency') || gasBooking.currency || 'EUR';
    ...
    $(document).on('click', '.gas-add-to-cart-btn:not(:disabled)', function() {
        ...
        var roomData = {
            ...
            currency: $roomWidget.data('currency') || gasBooking.currency || 'EUR'
        };
    ...
    if ($checkoutPage.length) {
        ...
        if (isGroupBooking) {
            ...
            var totalPrice = 0;
            var taxTotal = 0;
            var currency = cart[0].currency || 'EUR';
            ...
            $(document).off('click', '#gas-confirm-booking').on('click', '#gas-confirm-booking', function(e) {
                ...
                if (paymentMethod === 'card' && window.groupCheckoutData.stripe && window.groupCheckoutData.cardElement) {
                    ...
                    var currencyCode = (window.groupCheckoutData.currency || 'EUR').toLowerCase().replace(/[^a-z]/g, '').slice(0, 3) || 'eur';
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            var currency = window.groupCheckoutData.currency || 'EUR';
                            var roomsHtml = '';
        ...
        $(document).on('click', '.gas-payment-option:not(.disabled)', function() {
            ...
            if (paymentMethod === 'card' && checkoutData.stripeEnabled) {
                ...
                var currency = checkoutData.currency || 'EUR';
        ...
        function processCardPayment($btn) {
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    amount: paymentAmount,
                    currency: (checkoutData.currency || 'EUR').toLowerCase().replace(/[^a-z]/g, '').slice(0, 3) || 'eur',
                    booking_data: {
                        email: $form.find('[name="email"]').val(),
                        check_in: checkoutData.checkin,
                        check_out: checkoutData.checkout
